 ...
 * @author Antonio
 */
public class ProtocolsSnellDataxxPanel extends JPanelBGGradient {
    ...
    ProtocolsSnellDataxxPanel(Dataxx bean) {
setLayout(null);
        ...
        routerTable.getColumnModel().getColumn(1).setCellEditor(new BoundedIntegerCellEditor(bean.getDataxxPhysMapInfo().getDataxxPhysMapInfoNumMats().getMin(),bean.getDataxxPhysMapInfo().getDataxxPhysMapInfoNumMats().getMax()));
        routerTable.getColumnModel().getColumn(2).setCellEditor(new BoundedIntegerCellEditor(bean.getDataxxPhysMapInfo().getDataxxPhysMapInfoNumLvls().getMin(),bean.getDataxxPhysMapInfo().getDataxxPhysMapInfoNumLvls().getMax()));
        routerTable.getColumnModel().getColumn(3).setCellEditor(new BoundedIntegerCellEditor(bean.getDataxxPhysMapInfo().getDataxxPhysMapInfoNumOuts().getMin(),bean.getDataxxPhysMapInfo().getDataxxPhysMapInfoNumOuts().getMax()));
        routerTable.getColumnModel().getColumn(4).setCellEditor(new BoundedIntegerCellEditor(bean.getDataxxPhysMapInfo().getDataxxPhysMapInfoNumDass().getMin(),bean.getDataxxPhysMapInfo().getDataxxPhysMapInfoNumDass().getMax()));
...
//        menuItemAction.setFont(getFont().deriveFont(Font.BOLD));
    ...
    private class MyTableModel extends AbstractTableModel {
        ...
        public MyTableModel() {
            maxIns = bean.getDataxxPhysMapInfo().getDataxxPhysMapInfoNumIns().getVal();
            data = new Object[maxIns][6];
            Iterator it = bean.getDataxxPhysIns().iterateSnellDataxxIn();
            for (int i = 0; i < maxIns && it.hasNext(); i++) {
                SnellDataxxIn in = (SnellDataxxIn) it.next();
        ...
        private void save() {
            bean.getDataxxPhysIns().removeAllSnellDataxxIn();
            for (int i = 0; i < maxIns; i++) {
                SnellDataxxIn in = new SnellDataxxIn();
                ...
                bean.getDataxxPhysIns().addSnellDataxxIn(in);
            }
        }
    }
